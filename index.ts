import { ethers, providers, Wallet, utils, Transaction } from 'ethers'
import {
    FlashbotsBundleProvider,
    FlashbotsBundleResolution,
} from '@flashbots/ethers-provider-bundle'
import { exit } from 'process'
import * as dotenv from 'dotenv'
dotenv.config()

const FLASHBOTS_URL = 'https://relay-goerli.flashbots.net'
const LINK_ADDRESS = '0x326C977E6efc84E512bB9C30f76E30c160eD06FB'
const provider = new providers.JsonRpcProvider(process.env.GOERLI_URL)

function getEtherBalance(address: string, name: string) {
    provider.getBalance(address).then((balance) => {
        // convert a currency unit from wei to ether
        const balanceInEth = ethers.utils.formatEther(balance)
        console.log(`${name}: ${balanceInEth} ETH`)
    })
}

const main = async () => {
    if (
        process.env.SPONSOR_KEY === undefined ||
        process.env.VICTIM_KEY === undefined
    ) {
        console.error('Both SPONSOR_KEY and VICTIM_KEY must be set')
        exit(1)
    }

    const authSigner = Wallet.createRandom()
    const flashbotsProvider = await FlashbotsBundleProvider.create(
        provider,
        authSigner,
        FLASHBOTS_URL
    )

    const sponsor = new Wallet(process.env.SPONSOR_KEY, provider)
    const victim = new Wallet(process.env.VICTIM_KEY, provider)
    console.log(`Sponsor: ${sponsor.address}`)
    console.log(`Victim: ${victim.address}`)
    getEtherBalance(sponsor.address, 'Sponsor')
    getEtherBalance(victim.address, 'Victim')
    const abi = ['function transfer(address,uint256) external']
    const iface = new utils.Interface(abi)

    provider.on('block', async (blockNumber: number) => {
        console.log('Block number: ', blockNumber)
        const targetBlockNumber = blockNumber + 1
        const resp = await flashbotsProvider.sendBundle(
            [
                {
                    signer: sponsor,
                    transaction: {
                        chainId: 5,
                        type: 2,
                        to: victim.address,
                        value: utils.parseEther('0.01'),
                        maxFeePerGas: utils.parseUnits('250', 'gwei'),
                        maxPriorityFeePerGas: utils.parseUnits('2', 'gwei'),
                    },
                },
                {
                    signer: victim,
                    transaction: {
                        chainId: 5,
                        type: 2, // EIP-1559 type
                        to: LINK_ADDRESS,
                        gasLimit: '100000',
                        data: iface.encodeFunctionData('transfer', [
                            sponsor.address,
                            utils.parseEther('571.35691'),
                        ]),
                        maxFeePerGas: utils.parseUnits('250', 'gwei'),
                        maxPriorityFeePerGas: utils.parseUnits('2', 'gwei'),
                    },
                },
            ],
            targetBlockNumber
        )
        console.dir(`resp: ${JSON.stringify(resp)}`)

        if ('error' in resp) {
            console.log(resp.error.message)
            return
        }

        const resolution = await resp.wait()
        if (resolution === FlashbotsBundleResolution.BundleIncluded) {
            console.log(`Congrats, included in ${targetBlockNumber}`)
            exit(0)
        } else if (
            resolution === FlashbotsBundleResolution.BlockPassedWithoutInclusion
        ) {
            console.log(`Not included in ${targetBlockNumber}`)
        } else if (
            resolution === FlashbotsBundleResolution.AccountNonceTooHigh
        ) {
            console.log('Nonce too high, bailing')
            exit(1)
        }
    })
}

main()
